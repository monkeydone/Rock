...
class ExpandableTextView(context: Context, attrs: AttributeSet) : AppCompatTextView(context, attrs) {
    ...
    var titleColor: Int = R.color.purple_200.toArtColor()
    var contentColor: Int = R.color.teal_200.toArtColor()
    var titleSize = 12
    var contentSize = 12
...
//        b.setSpan(BackgroundColorSpan(Color.BLUE),0,content.length,Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        b.setSpan(LineHeightSpan.Standard(dpToPx(lineHeight).toInt()), 0, content.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        return b
    }
    ...
    fun addMessage(title: String, data: String) {
        ...
        b.setSpan(ForegroundColorSpan(titleColor), 0, title.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
//        b.setSpan(BackgroundColorSpan(Color.BLUE),0,title.length,Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        ...
        b.setSpan(AbsoluteSizeSpan(titleSize, true), 0, title.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
//        b.setSpan(LineHeightSpan.Standard(com.windimg.giant.utils.ViewUtils.dpToPx(20.0f).toInt()), 0, title.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        ...
        b2.setSpan(ForegroundColorSpan(contentColor), 0, data.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        b2.setSpan(AbsoluteSizeSpan(contentSize, true), 0, data.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    private fun getEndString(expend: Boolean): SpannableStringBuilder {
        ...
        val b = SpannableStringBuilder(content)
        b.setSpan(AbsoluteSizeSpan(btnSize, true), 0, content.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        b.setSpan(ForegroundColorSpan(btnColor), 0, content.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        b.setSpan(LineHeightSpan.Standard(dpToPx(btnHeight).toInt()), 0, content.length, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE)
        return b
    }
    ...
    private fun getStaticLayout(text: CharSequence): StaticLayout {
        return StaticLayout(text, paint, contentWidth, Layout.Alignment.ALIGN_NORMAL, lineSpacingMultiplier, lineSpacingExtra, includeFontPadding)
    }
